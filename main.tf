...
module "ws1" {
  ...
  the_workspace_id = data.tfe_workspace_ids.external_ids["aws-shared-credentials-file"]
}

module "ws2" {
  source = "./the_variables"
  #name = "terraform-null-localexec"
  the_workspace_id = data.tfe_workspace_ids.external_ids["terraform-null-localexec"] #["ws2"]
}
